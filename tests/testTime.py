...
def nutc(tdb):
    ...
    el = RadPerArcSec * (485866.733 + ((1325 * ArcSecPerRev) + 715922.633 + (31.310 + (0.064 * t)) * t) * t)

    #  Mean longitude of the sun minus mean longitude of the sun's perigee
    elp = RadPerArcSec * (1287099.804 + ((99 * ArcSecPerRev) + 1292581.224 + (-0.577 - 0.012 * t) * t) * t)

    #  Mean longitude of the moon minus mean longitude of the moon's node
    f = RadPerArcSec * (335778.877 + (1342 * ArcSecPerRev + 295263.137 + (-13.257 + 0.011 * t) * t) * t)

    #  Mean elongation of the moon from the sun
    d = RadPerArcSec * (1072261.307 + (1236 * ArcSecPerRev + 1105601.328 + (-6.891 + 0.019 * t) * t) * t)

    #  Longitude of the mean ascending node of the lunar orbit on the
    #   ecliptic, measured from the mean equinox of date
    om = RadPerArcSec * (450160.280 + (-5 * ArcSecPerRev - 482890.539 + (7.455 + 0.008 * t) * t) * t)
...
def gmstFromUT1(ut1):
    ...
    return coordConv.wrapPos(
        ...
        (24110.54841 + (8640184.812866 + (0.093104-(6.2e-6*jc))*jc)*jc)*0.0041666666666666666)
...
class TestTime(unittest.TestCase):
    ...
    def testJulianEpochFromTAI(self):
        ...
        for tai in (4232.89, 20000.32, 56350.03, 74222.9):
            ...
            self.assertAlmostEqual(coordConv.taiFromJulianEpoch(
                coordConv.julianEpochFromTAI(tai)), tai, places=5)
    ...
    def testBesselianEpochFromTAI(self):
        ...
        for tai in (4232.89, 20000.32, 56350.03, 74222.9):
            ...
            self.assertAlmostEqual(coordConv.taiFromBesselianEpoch(
                coordConv.besselianEpochFromTAI(tai)), tai, places=5)
