...
JulianSysList = (coordConv.ICRSCoordSys, coordConv.FK5CoordSys,
                 coordConv.GalCoordSys, coordConv.AppGeoCoordSys)
                    ...
                    for polAng in (-20, 25, 89):
                        for dist in (0, 0.01, 1):
                            for pmRA in (0, -3):
                                for pmDec in (0, 5):
                                    for radVel in (0, 7):
                                        fromCoord = coordConv.Coord(
                                            equatAng, polAng, dist, pmRA, pmDec, radVel)
            ...
            for date in (0, 1000.5, 2001):
                if date == 0 and csysName in ("fk4", "fk5"):
                    if self.assertRaises(Exception):
                        coordConv.makeCoordSys(csysName, date)
                else:
    ...
    def testNoneAndOtherCoordSys(self):
        ...
        for nullSys in (
            ...
            for csysName in FullNameList:
                ...
                with self.assertRaises(Exception):
                    nullSys.convertFrom(otherSys, fromCoord, site)
                with self.assertRaises(Exception):
                    otherSys.convertFrom(nullSys, fromCoord, site)
    ...
    def testInvalidCoordSys(self):
        ...
        for csysName in ("foo", "bar", "icrsx", "null"):
            with self.assertRaises(ValueError):
                coordConv.makeCoordSys(csysName, 0)
